...
public class MainBoardDao {
	 ...
	 * 게시글 목록 가저오기
	 */
	public void getBoardList(Connection conn, ArrayList<MainBoardDto> allList)
			throws SQLException {
		String sql = "SELECT mboard_num, business_id ,mcategory_num, scategory_num,mboard_title, mboard_score, mboard_img, mboard_info,mboard_content, NVL(score1,0) as score ,count "
				+"FROM mainboard m "
				+"left outer JOIN (SELECT trunc(AVG(r.review_score)) as score1 ,COUNT(review_Num) as count, mboard_num FROM  mainboard m JOIN review  r USING(mboard_num) GROUP BY mboard_num) s "
				+"USING(mboard_num) "
	...
	public boolean checkreview(String memberId, String mboardNum) {
		String sql = "select * from review where mboard_num = ? and member_Id = ?";
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
